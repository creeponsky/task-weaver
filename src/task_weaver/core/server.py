...
from ..models.server_models import ResourceType, Server, ServerStatus, ServerTier, ServerOperationStats
...
class ServerManager:
    ...
    def check_has_idle(self, server_type: str = None):
        ...
        if server_type:
            return any(server for server in idle_servers 
    ...
    async def register_server(self, ip: str, server_name: str, description, tier: ServerTier, 
        ...
        async with self._lock:
            ...
            if old_server:
                ...
                server = Server(ip=ip, server_name=server_name, description=description, 
                              tier=tier, available_task_types=available_task_types if available_task_types else [], 
    ...
    async def get_idle_server(self, available_task_type: str = None, 
                            task_resource_type: ResourceType = None) -> Optional[Server]:
        start_time = time.time()
        ...
        async with self._lock:
            ...
            if available_task_type:
                ...
                idle_servers = [s for s in idle_servers if s.check_available_task_type(available_task_type)]
            
            if task_resource_type:
                idle_servers = [s for s in idle_servers if s.server_type == task_resource_type]
    ...
    async def add_running_server(self, ip: Union[str, None] = None, server_name: Union[str, None] = None):
        start_time = time.time()
        async with self._lock:
